#!/usr/bin/env python3
"""Conversation-derived Cyberdash Red Kivy prototype.

This is a faithful reconstruction of the dashboard code supplied in the chat,
not a byte-for-byte copy recovered from the Raspberry Pi. It targets a fixed
640x480 development window and keeps the Civic and audio visualizer simulated.
"""
...
class CivicOutline(Widget):
    """Temporary etched-style side profile used before the 360° player."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        with self.canvas:
            Color(*RED)
            self.glow_line = Line(points=[], width=3.0, close=False)
            Color(*WHITE)
            self.body_line = Line(points=[], width=1.15, close=False)
            self.front_wheel = Line(circle=(0, 0, 1), width=1.2)
            self.rear_wheel = Line(circle=(0, 0, 1), width=1.2)
            Color(*RED)
            self.front_hub = Line(circle=(0, 0, 1), width=1.0)
            self.rear_hub = Line(circle=(0, 0, 1), width=1.0)
            Color(*DARK_GREY)
            self.ground_line = Line(points=[], width=1.0)
        self.bind(pos=self.update_canvas, size=self.update_canvas)
    ...
    def update_canvas(self, *_):
        ...
        self.glow_line.points = profile
        self.body_line.points = profile

        wheel_radius = min(w, h) * 0.095
        hub_radius = wheel_radius * 0.35
        rear_x, front_x = x + w * 0.25, x + w * 0.76
        wheel_y = y + h * 0.30
        self.rear_wheel.circle = (rear_x, wheel_y, wheel_radius)
        self.front_wheel.circle = (front_x, wheel_y, wheel_radius)
        self.rear_hub.circle = (rear_x, wheel_y, hub_radius)
        self.front_hub.circle = (front_x, wheel_y, hub_radius)
        self.ground_line.points = [
            x + w * 0.08,
            y + h * 0.18,
            x + w * 0.92,
            y + h * 0.18,
...
class Dashboard(FloatLayout):
    ...
    def create_vehicle_panel(self):
        ...
        self.add_widget(
            CivicOutline(
                size_hint=(None, None),
                size=(panel.width - dp(30), panel.height - dp(55)),
                pos=(panel.x + dp(15), panel.y + dp(9)),
            )
        )
        self.add_widget(
            fixed_label(
