...
- code-drawn neon-red floor glow synchronized to each rotation frame
...
The red underglow is generated by Kivy rather than baked into the approved
Civic PNGs. `underglow_tracking.tsv` stores the floor position, center and
width for every frame. The glow therefore narrows for front/rear views, widens
for side views, and changes on the exact same frame as the Civic. It fades in
with the car and has no independent animation timer. Set `UNDERGLOW_ENABLED`
to `False` to disable it, or lower `UNDERGLOW_OPACITY` for a subtler effect.
