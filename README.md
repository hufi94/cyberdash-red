...
- preserved no-red source set under `assets/civic_frames_outline`
- strong red underbody bloom generated from a hidden lower-center source
- downward reflection with no visible red strip or red on the body lines
- visible bloom locked to the Civic's smoothed body-center path in all frames
- smooth projected footprint that expands at side views and narrows front/rear
- no independent ground line, geometry, transform, or animation timer
...
- `build_civic_bottom_glow.py` — rebuilds the attached bottom-glow sequence
...
The dashboard loads `assets/civic_frames_bottom_glow`. Every image contains
the approved white Civic plus a strong red bloom generated by a light source
hidden inside the lower-center silhouette. The hidden source cannot appear as
a red line; only its soft reflection escapes below the car and spreads
downward. The builder follows the Civic's smoothed frame-by-frame body center,
then compensates for the changing body and wheel silhouette so the visible
light follows the car instead of remaining fixed on the screen. Because car
and glow are one texture, they share the same frame, scaling, position, fade
and rotation with no second Kivy layer that can drift away. The original
no-red frames remain preserved separately.
...
After rebuilding the no-red frames, generate the attached underglow frames:

```bash
python build_civic_bottom_glow.py \
    --source assets/civic_frames_outline \
    --output assets/civic_frames_bottom_glow
```

The glow builder places a hidden source inside the lower body separately in
all 220 images, masks the source behind the Civic, fades away the contact line,
and lets only a strong soft reflection spread downward. It also uses a smooth
projected footprint and corrects the visible glow to the Civic's smoothed body
center at every angle. Every original Civic pixel remains unchanged.
...
The source silver frames are not committed. Both the approved no-red set and
the derived bottom-glow set are committed, so a fresh Pi clone is complete.
