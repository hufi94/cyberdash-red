...
- visible bloom locked to one fixed turntable pivot throughout all 220 frames
...
downward. The builder compensates for the changing body silhouette so the
visible light stays centered on one fixed rotation pivot instead of following
the car's shifting bounding box. Because car and glow are one texture, they
share the same frame, scaling, position, fade and rotation with no second Kivy
layer that can drift away. The original no-red frames remain preserved
separately.
...
projected footprint and corrects the visible glow back to the fixed turntable
pivot at every angle. Every original Civic pixel remains unchanged.
