...
- generated red glow attached to each frame's actual bottom contour
...
the approved white Civic plus a red halo generated from that frame's actual
lowest visible body pixels. Because car and glow are one texture, they share
the same frame, scaling, position, fade and rotation with no second Kivy layer
that can drift away. The original no-red frames remain preserved separately.
...
The glow builder traces the real bottom edge separately in all 220 images,
adds a compact red bloom in transparent pixels immediately outside that edge,
and leaves every original Civic pixel unchanged.
